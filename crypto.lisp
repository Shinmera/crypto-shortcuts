...
(defgeneric encrypt (text key &key cipher mode IV to normalize-key)
  (:method ((text string) key &rest args &key cipher mode IV to normalize-key)
    (declare (ignore cipher mode iv to normalize-key))
    (apply #'encrypt (to-octets text) key args))
  (:method ((text vector) key &key (cipher :aes) (mode :ecb) (IV (ironclad:make-random-salt)) (to :base64) normalize-key)
...
(defgeneric decrypt (text key &key cipher mode IV from normalize-key)
  (:method ((text string) key &rest args &key cipher mode IV (from :base64) normalize-key)
    (declare (ignore cipher mode IV normalize-key))
    (apply #'decrypt (code from :octets text) key args))
  (:method ((text vector) key &key (cipher :aes) (mode :ecb) IV from normalize-key)
    (declare (ignore from))
    (let* ((key (normalize-key normalize-key key))
           (cipher (get-cipher key :cipher cipher :mode mode :IV IV)))
